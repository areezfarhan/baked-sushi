  ...
  if (cartItems.length === 0) {
    return (
      <div className="min-h-screen bg-warm-50 flex flex-col items-center justify-center text-center px-6">
        ...
        <p className="text-gray-500 mb-8 max-w-xs mx-auto">Add some delicious baked sushi to get started.</p>
        <Link to="/" className="btn-secondary w-full max-w-xs py-3 text-center">Go to Menu</Link>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-warm-50 pb-32">
      ...
      <div className="max-w-3xl mx-auto px-4 py-6 space-y-6">
        ...
        {/* Order Summary Card */}
        <div className="bg-white rounded-2xl shadow-soft p-5 border border-warm-100">
          ...
          <ul className="space-y-3 mb-4">
            {cartItems.map((item, index) => (
              <li key={index} className="flex items-center gap-3 py-2 border-b border-warm-100 last:border-0 last:pb-0">
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="text-sm font-semibold text-text-main truncate">{item.product.name}</h3>
                  <p className="text-xs text-gray-500">Qty: {item.quantity}</p>
                </div>
                <span className="font-bold text-text-main">RM{item.product.price * item.quantity}</span>
              </li>
            ))}
          </ul>
          
          <div className="flex justify-between items-center pt-3 border-t-2 border-warm-200">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="bg-white rounded-2xl shadow-soft p-5 border border-warm-100 space-y-4">
            <h2 className="text-lg font-bold text-text-main">Customer Details</h2>
                ...
                <button
                  ...
                  className={`py-3 px-4 rounded-xl font-semibold transition-all ${
                    deliveryType === 'pickup'
                      ...
                      : 'bg-warm-50 text-text-main border border-gray-200 hover:bg-gray-100'
                  }`}
                >
                  Pickup
                </button>
          ...
          {/* Payment Section */}
          <div className="bg-white rounded-2xl shadow-soft p-5 border border-warm-100 space-y-5">
            <h2 className="text-lg font-bold text-text-main">Payment</h2>
            
            {/* Payment Info Card */}
            <div className="bg-gradient-to-br from-warm-50 to-warm-100 p-5 rounded-xl border border-warm-200">
              <div className="flex items-center gap-2 mb-4">
                <svg className="w-5 h-5 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z" />
                </svg>
                <h3 className="text-base font-bold text-text-main">DuitNow QR / Bank Transfer</h3>
              </div>
              ...
              <div className="space-y-3 text-sm">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-text-muted text-xs font-medium">Bank Name</p>
                    <p className="text-text-main font-bold">Maybank</p>
                  </div>
                </div>
                <div className="flex items-start gap-3">
                  <svg className="w-5 h-5 text-primary mt-0.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <div>
                    <p className="text-text-muted text-xs font-medium">Account Number</p>
                    <p className="text-text-main font-bold">1234567890</p>
                  </div>
                </div>
                <div className="flex items-start gap-3">
                  <svg className="w-5 h-5 text-primary mt-0.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <div>
                    <p className="text-text-muted text-xs font-medium">Account Name</p>
                    <p className="text-text-main font-bold">Baked Sushi Owner</p>
                  </div>
                </div>
              </div>
            ...
            <div className="space-y-3">
              ...
              <div className="w-full">
                <label className="flex flex-col items-center justify-center w-full h-36 border-2 border-dashed border-warm-200 rounded-xl cursor-pointer hover:bg-warm-50 hover:border-primary transition-all bg-white">
                  <div className="flex flex-col items-center justify-center pt-5 pb-6 px-4">
                    ...
                    {receipt && (
                      <p className="text-xs text-text-muted text-center break-all px-2">
                        {receipt.name}
                      </p>
                    )}
                    <input
